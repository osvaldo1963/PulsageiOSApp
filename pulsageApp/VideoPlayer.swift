...
class VideoPlayer: UIViewController {
    ...
    fileprivate func getComments() {
        ...
        query.findObjectsInBackground { (data, error) in
            ...
            DispatchQueue.main.async {
                self.vpTable.reloadSections(IndexSet(integer: 1), with: .fade)
    ...
    /*
    @objc fileprivate func voteBest() {
        guard let userid = self.currentUser?.objectId else {return}
        if self.currentUserVote {
            let query = PFQuery(className: "VoteBest")
            query.whereKey("videoId", equalTo: self.id)
            query.whereKey("userId", equalTo: userid)
            query.getFirstObjectInBackground(block: { (result, error) in
                result?.deleteInBackground(block: { (success, error) in
                    if success {
                        self.currentUserVote = false
                         self.voteColorChange = UIColor.gray
                        DispatchQueue.main.async {
                            self.vpTable.reloadData()
                        }
                    }
                })
            })
        } else {
            let query = PFObject(className: "VoteBest")
            query["videoId"] = self.id
            query["userId"] = userid
            query.saveInBackground { (success, error) in
                if error == nil {
                    if success {
                        self.currentUserVote = true
                         self.voteColorChange = UIColor.orange
                        DispatchQueue.main.async {
                            self.vpTable.reloadData()
                        }
                    }
    ...
    fileprivate func checkCurrentuserVote() {
        guard let uid = PFUser.current()?.objectId else {return}
        ...
        query.getFirstObjectInBackground { (result, error) in
            if error == nil {
                guard let user = result?.objectId else {return}
                if user == ""  {
                    self.currentUserVote = false
                   self.voteColorChange = UIColor.gray
                    DispatchQueue.main.async {
...
extension VideoPlayer: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.section == 0 {
            ...
            if self.buttonPressed == "Comments" {
                guard let commentsCell = tableView.dequeueReusableCell(withIdentifier: self.commentid, for: indexPath) as? CommentCell else {return UITableViewCell()}
    ...
    func textFieldShouldReturn(_ textField: UITextField) -> Bool {
        ...
        if self.currentUser != nil {
            if textField.text != ""{
                ...
                query["userId"] = self.currentUser?.objectId
                query["videoId"] = self.videdata.objectId!
                query.saveInBackground { (success, error) in
                    if success {
                        self.getComments()
                        self.textfield.text = ""
